...
prop_candidates_correct sud = and [ prop_candidates_correct' (candidates sud (y,x))
                                    (r!!y ++ (transpose r)!!x ++ (findBlock sud (y,x))) | (y,x) <- (blanks sud) ]
  where r     = rows sud

prop_candidates_correct' :: [Int] -> [Maybe Int] -> Bool
prop_candidates_correct' []      _         = True -- If there's no candidates, then the test succeeds.
prop_candidates_correct' (l:ls) cells
  | length (intersect cells [Just l]) == 0 = prop_candidates_correct' ls cells -- If candidate works for given position, try the next one.
